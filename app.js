...
bot.on('message', function(msg) {
  ...
  if (/^\/[Mm](ovie)? (.+)$/g.test(message)) {
    if(isAuthorized(user.id)){
       var movieName = /^\/[Mm](ovie)? (.+)/g.exec(message)[2] || null;
       return radarr.sendMovieList(movieName);
    } else {
       return replyWithError(user.id, new Error(i18n.__('notAuthorized')));     
    }
  }
  if (/^\/[Ss](eries)? (.+)$/g.test(message)) {
    if(isAuthorized(user.id)){
       var movieName = /^\/[Ss](eries)? (.+)/g.exec(message)[2] || null;
       return sonarr.sendSeriesList(movieName);
    } else {
       return replyWithError(user.id, new Error(i18n.__('notAuthorized')));     
    }
  }
